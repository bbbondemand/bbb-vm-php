...
*
*  This allows customers to create and manage their own, dedicated virtual servers running BBB. So the '/bigbluebutton/api' end *  point is used to manage meetings using a standard BBB library or integration; the /vm endpoint is used to manage your own 
*  virtual machines - and you would then use a BBB library to interact with the actual BBB instance running on each machine.
*
...
class VM{
    ...
    {
        $this->customerID       = getenv('VM_CUSTOMER_ID');
        $this->customerApiToken = getenv('VM_CUSTOMER_API_TOKEN');
        $this->apiServerBaseUrl = "https://api.bbbondemand.com/v1";
        $this->urlBuilder       = new UrlBuilder($this->customerID, $this->customerApiToken,$this->apiServerBaseUrl);
    }
    ...
    public function executeApiCall($url, $requestType="GET", $paramJson=[], $getJson=true) {
        $client = new Client([
            'headers' => $this->getApiCallHeaders()
        ]);

        try {
            $responseJson = $client->request($requestType, $url, $this->getApiCallParams($paramJson))->getBody()->getContents();
